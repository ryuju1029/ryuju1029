      ...
      if (existBeforeChar) {
        if (allowBeforeSpace) {
          if (beforeChar !== " " && isJapaneseChar(beforeChar)) {
            report(node, new RuleError("リンクの前にスペースを入れてください。", {
              index: -1,
              // before `
          ...
          if (beforeChar === " " && isJapaneseChar(beforeBeforeChar)) {
            report(node, new RuleError("リンクの前にスペースを入れません。", {
              index: -1,
              // before `
      ...
      if (existAfterChar) {
        if (allowAfterSpace) {
          if (afterChar !== " " && isJapaneseChar(afterChar)) {
            report(node, new RuleError("リンクの後にスペースを入れてください。", {
          ...
          if (afterChar === " " && isJapaneseChar(afterAfterChar)) {
            report(node, new RuleError("リンクの後にスペースを入れません。", {
