...
function removeRangeFromString(text, regExpStrings) {
    var patterns = regExpStrings.map(function (pattern) {
        return regexp_string_matcher_1.createRegExp(pattern);
    });
    var result = text;
...
var reporter = function (context, options) {
    ...
    return _a = {},
        _a[Syntax.Paragraph] = function (node) {
            ...
            var paragraph = sentence_splitter_1.splitAST(node);
            paragraph.children
                .filter(function (sentence) { return sentence.type === sentence_splitter_1.Syntax.Sentence; })
                ...
                if (sentenceLength > maxLength) {
                    ...
                    report(filteredSentence, new RuleError("Line " + startLine + " sentence length(" + (sentenceLength !== actualTextLength
                        ? sentenceLength + ", original:" + actualTextLength
                        : sentenceLength) + ") exceeds the maximum sentence length of " + maxLength + ".\nOver " + (sentenceLength - maxLength) + " characters."));
                }
            });
        },
        _a;
